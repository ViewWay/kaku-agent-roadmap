...
## 用户交互设计文档 (AGENT_ROADMAP.md 新增章节)

- [x] 通用交互规范 — 快捷键体系 (22 快捷键 + 4 平台全局快捷键)
- [x] 通用交互规范 — 输入模式 (对话/命令/Vi 三种模式)
- [x] 通用交互规范 — @ 引用语法 (7 种引用: @file, @folder, @git-diff, @terminal, @image, @url)
- [x] 通用交互规范 — Slash 命令完整列表 (16 个命令)
- [x] 通用交互规范 — 审批流程 UI (单步/批量/权限记忆三种审批模式)
- [x] 通用交互规范 — 状态指示器体系 (8 种 Agent 状态)
- [x] 通用交互规范 — 错误展示规范 (10 种错误类型 + 操作)
- [x] 通用交互规范 — 配置入口体系 (8 种配置入口)
- [x] G1 交互细节 — 8 个任务的完整用户感知设计
- [x] G2 交互细节 — 4 个任务的完整用户感知设计 (含沙箱配置、权限规则示例)
- [x] H 交互细节 — 7 个任务的完整交互流程 (含子代理生命周期、消息传递、聚合面板)
- [x] I 交互细节 — 5 个任务的完整交互设计 (含 Hook/MCP/Skill/LSP/配置管理命令)
- [x] J 交互细节 — 7 个任务的完整交互设计 (含 Diff 编辑器、多 Tab、通知、ka 命令)
- [x] K 交互细节 — 6 个任务的完整交互设计 (含 Plugin、Auto Mode、Plan、会话、Feature Flag、Workflow)
